    ...
    if (dbUser.linkedInAccessToken) {
        try {
            ...
            if (urn && urn.startsWith("urn:li:organization:")) {
                ...
                // Default to profile - Use OIDC userinfo for modern scopes
                ...
                if (userInfoRes.ok) {
                    ...
                    if (userInfo.sub) {
                        const correctUrn = `urn:li:person:${userInfo.sub}`;
                        if (urn !== correctUrn) {
                            try {
                                await prisma.user.update({
                                    where: { id: dbUser.id },
                                ...
                                urn = correctUrn;
                                console.log(`[Social Status] Auto-fixed LinkedIn URN via OIDC for user ${dbUser.id} to ${correctUrn}`);
                            } catch (updateErr) {
                        ...
                        if (data.id) {
                            ...
                            if (urn !== correctUrn) {
                                try {
                                    ...
                                    urn = correctUrn;
                                    console.log(`[Social Status] Auto-fixed LinkedIn URN via Legacy for user ${dbUser.id} to ${correctUrn}`);
                                } catch (updateErr) {
                                    console.error("[Social Status] Failed to auto-fix LinkedIn URN", updateErr);
                                }
                            }
