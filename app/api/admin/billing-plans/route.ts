...
// GET: List all plans
async function getBillingPlans(request: NextRequest) {
...
async function createBillingPlan(request: NextRequest) {
    const { userId } = await auth();
    if (!userId) {
...
export const GET = withErrorHandling(getBillingPlans, "GET /api/admin/billing-plans");
export const POST = withErrorHandling(createBillingPlan, "POST /api/admin/billing-plans");
