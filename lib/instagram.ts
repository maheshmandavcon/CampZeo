import { validateMediaUrl, isVideoUrl, validateInstaMediaUrl } from './media-utils';
  ...
  try {
    ...
    if (isCarousel) {
      ...
      for (const originalUrl of mediaList) {
        ...
        if (isVideo) {
          childParams.append("media_type", "VIDEO"); // For carousel items, VIDEO is often still the correct internal type, but we ensure consistency
        }

        let childId = '';
        if (isVideo) {
          childParams.append("video_url", mediaUrl);
        } else {
    ...
    // ============================================================
      ...
      const validation = validateInstaMediaUrl(originalUrl);
      const mediaUrl = validation.url;
      ...
      if (isVideo) {
        // Meta has deprecated 'VIDEO' type for single posts in favor of 'REELS'
      ...
      if (!response.ok) {
        ...
        console.error(`[IG_DEBUG] Media Container Creation Failed:`, JSON.stringify(err, null, 2));
...
async function waitForInstagramMediaProcessing(
  containerId: string,
  accessToken: string,
  timeout: number = 600000 // 10 minute timeout for video processing
): Promise<void> {
  const startTime = Date.now();
  const pollInterval = 10000; // Increased poll interval to 5s
  ...
  while (Date.now() - startTime < timeout) {
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(`Instagram Processing Error: Instagram API Error (400) 'Invalid container or token'`);
      }
    } catch (e: any) {
